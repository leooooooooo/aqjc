...
-(IBAction)signinbt{
    //1确定地址NSURL
    NSString *urlString = [NSString stringWithFormat:Server,@"Mobile/home/Login"];

    NSString *bodyStr = [NSString stringWithFormat:@"username=%@&password=%@", self.ID.text, self.PW.text];

    [Post getAsynchronousRequestDataJSONSerializationWithURL:urlString withHTTPBody:bodyStr Delegate:self];
}
